...
size_t u8c_encode_utf8_length(uint_least32_t const* const restrict source, size_t const count) {
	size_t length = 0x0u;

	for (ptrdiff_t index = 0x0; index < (ptrdiff_t)count; ++index) {
		uint_least32_t code_point = source[index];

		// We need to check if the input is valid, as the
		// its invalidity is replaced with the replacement
		// character, which may have a different length.
		...
		if (code_point >= UINT32_C(0x10000)) {
			length += 0x4u;
		} else if (code_point >= UINT32_C(0x800)) {
			length += 0x3u;
		} else if (code_point >= UINT32_C(0x80)) {
			length += 0x2u;
		} else {
			++length;
		}
